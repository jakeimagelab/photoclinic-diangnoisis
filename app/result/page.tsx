...
const CASES = [
  {
    name: "트리니티여성의원",
    dept: "산부인과",
    desc: "따뜻하고 부드러운 이미지로 리브랜딩",
  },
  {
    name: "르블랑치과",
    dept: "치과",
    desc: "공간의 감성을 사진으로 완성",
  },
  {
    name: "진주마음우산정신건강의학과",
    dept: "정신건강의학과",
    desc: "병원의 감성을 섬세하게 기록",
  },
];
...
export default function ResultPage() {
  ...
  const sendReportEmail = async () => {
    if (!answers.email) {
      setMailStatus("error");
      setMailMessage(
        "이메일 주소가 없어 메일을 보낼 수 없습니다. 처음부터 다시 진행해 이메일을 입력해주세요."
      );
      return;
    }
  ...
  return (
    <main className="relative min-h-screen bg-base">
      ...
      <section className="max-w-[980px] mx-auto px-9 pt-16 pb-32">
        <div>
          <div className="text-green text-[13px] tracking-wider2 mb-3.5 font-semibold">
            DIAGNOSIS COMPLETE
          </div>

          <h1 className="text-[26px] md:text-[48px] leading-[1.28] text-primary font-bold tracking-tight">
            <span className="text-green">
              {answers.hospitalName ?? "병원"}
            </span>
            은
            <br />
        ...
        <div className="mt-14 grid md:grid-cols-[1.1fr_0.9fr] gap-6">
          <section className="rounded-[14px] border border-green-line bg-elev p-7 md:p-9">
            <div className="text-green text-xs tracking-wider2 mb-4 font-semibold">
              PHOTO PRESCRIPTION
            </div>

            <h2 className="text-[22px] text-primary font-bold tracking-tight">
              병원사진 촬영 처방전
            </h2>
          ...
          <section className="rounded-[14px] border border-line-soft bg-white/60 p-7 md:p-9">
            <div className="text-orange text-xs tracking-wider2 mb-4 font-semibold">
              CHECK POINT
            </div>
            ...
            <ul className="mt-4 space-y-3">
              {rec.risks.map((risk) => (
                <li
                  key={risk}
                  className="flex gap-3 text-sm text-secondary leading-[1.7]"
                >
        ...
        <div style={{ marginTop: 72 }}>
          <div className="text-green text-xs tracking-wider2 mb-[22px] font-semibold">
            RECOMMENDED SHOTS
          </div>

          <h3 className="text-[22px] text-primary font-bold tracking-tight mb-6">
            우선 확보해야 할 사진 구성
          </h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {rec.neededShots.map((shot, i) => (
              <div
                key={shot}
                className="rounded-[12px] border border-line-soft bg-elev p-5"
              >
                <div className="text-orange text-[11px] tracking-wider2 font-bold">
                  {String(i + 1).padStart(2, "0")}
                </div>
                <div className="mt-3 text-sm text-primary font-semibold leading-[1.55]">
                  {shot}
                </div>
              </div>
            ))}
        ...
        <div style={{ marginTop: 88 }}>
          ...
          <div
            className={`grid gap-6 ${
              rec.packages.length === 2 ? "md:grid-cols-2" : "md:grid-cols-1"
            }`}
          >
        ...
        <div
          className="rounded-[14px] border border-green-line bg-elev p-9 md:p-12"
          style={{ marginTop: 72 }}
        >
          <div className="text-green text-xs tracking-wider2 mb-[22px] font-semibold">
            NEXT STEP
          ...
          <h3 className="text-xl md:text-[28px] text-primary font-bold leading-[1.4] tracking-tight">
            진단 결과는 상담을 위한 기초 자료로 활용됩니다.
          </h3>

          <p className="mt-4 text-secondary leading-[1.8]">
            포토클리닉이 병원 상황에 맞는 촬영 방향을 제안드리겠습니다.
          </p>
          ...
          <label className="mt-7 flex items-start gap-3 cursor-pointer select-none">
            ...
            <span className="text-sm text-secondary">
              전체 진단 요약과 사전 컨설팅 자료를{" "}
              {answers.email ? `이메일(${answers.email})로` : "이메일로"}{" "}
              받아보겠습니다.
            </span>
          </label>
          ...
          {mailMessage && (
            <p
              className={`mt-5 text-sm leading-[1.7] ${
                mailStatus === "error" ? "text-orange" : "text-green"
              }`}
            >
              {mailMessage}
            </p>
          ...
          {!answers.email && (
            <p className="mt-5 text-sm text-muted leading-[1.7]">
              이메일을 입력하지 않은 경우 메일 전송은 비활성화됩니다.
            </p>
          )}
        </div>
